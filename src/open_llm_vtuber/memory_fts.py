...
def search(conf_uid: str, query: str, k: int = 3) -> List[str]:
    ...
    try:
        ...
        if match is None:
            return []
        return [item["snippet"] for item in search_detailed(conf_uid, query, k)]
    except Exception as e:
        logger.warning(f"[fts] search failed for {conf_uid}: {e}")
        return []


def search_detailed(conf_uid: str, query: str, k: int = 3) -> list[dict]:
    """FTS results with transcript provenance for UI links and hybrid ranking."""
    try:
        ...
        try:
            rows = conn.execute(
                "SELECT text, role, ts, src FROM snippets WHERE snippets MATCH ? "
                "ORDER BY rank LIMIT ?",
                (match, k),
            ).fetchall()
        finally:
        ...
        results = []
        seen = set()
        for rank, (text, role, timestamp, source) in enumerate(rows):
            ...
            results.append(
                {
                    ...
                    "history_uid": os.path.splitext(
                        os.path.basename(str(source or ""))
                    )[0],
                    "score": 1.0 / (rank + 1),
                    "method": "keyword",
                    "snippet": _format_snippet(text, str(role or "")),
                }
            )
        return results
    except Exception as e:
        logger.warning(f"[fts] detailed search failed for {conf_uid}: {e}")
        return []
