...
def load_core_memory(conf_uid: str) -> str:
    ...
    try:
        p = _path(conf_uid)
        if os.path.isfile(p):
            with open(p, encoding="utf-8") as memory_file:
...
def _clean_consolidation_output(content: str) -> str:
    """Remove prompt scaffolding that small local models sometimes echo back."""
    cleaned: list[str] = []
    for raw_line in (content or "").strip().strip("`").splitlines():
        line = raw_line.strip()
        if not line or line in {
            ...
            continue
        if line.startswith(("AI 回：", "AI回：", "AI 回复：", "AI回复：")):
            continue
        if line.startswith(("使用者說：", "使用者说：", "用户说：")):
            fact = line.split("：", 1)[1].strip()
            if fact:
                cleaned.append(f"- {fact}")
            continue
        cleaned.append(line)
    return "\n".join(cleaned).strip()
...
async def consolidate_core_memory(
    ...
    try:
        ...
        # 防呆：LLM 可能回空或加引號
        new_mem = _clean_consolidation_output(new_mem)
        from .memory_records import active_memory_text, update_from_turn

        update_from_turn(conf_uid, user_input, new_mem, source_history_uid)
        structured_memory = active_memory_text(conf_uid, cap)
        if structured_memory:
            new_mem = structured_memory
